...
pamela = UnPersonaje 
    "Pamela" 
    (lluviaDeTuercas "sanadoras" False)
    torretaCurativa 
    True 
    9600 
...
granadaDeEspinas radioDeExplosion contrincante
  | radioDeExplosion > 3 && vida contrincante < 800 = contrincante {esSuperPoderActivo = False, vida = 0}
...
torretaCurativa aliado = aliado {esSuperPoderActivo = True, vida = doble (vida aliado)}